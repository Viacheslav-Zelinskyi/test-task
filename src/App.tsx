  ...
  function Clients()
{
  ...
  if(status === 'success')
  {
    ...
    return <Client key={index} id={val.id} firstName={val.firstName} lastName={val.lastName} imgURL={val.avatarUrl} phone={val.phone} className={'h-12 w-12 rounded-full'}></Client>;
    });
    return clients;
  }
 if(status === 'loading')
  ...
  <div style={{position: 'fixed', right: '5%', bottom: '10%'}}>
    <button type="button" onClick={()=>{setDisplayModalAdd('flex')}} style={{boxShadow: '0 5 10px rgba(0,0,0,0.5)'}} className="flex flex-col items-center justify-center border border-transparent rounded-full shadow-sm text-sm font-medium text-white bg-green-500 hover:bg-green-600 focus:outline-none h-20 w-20">
